...
| **Matrix & Linear Algebra**           | LU decomposition, LUP solver, Strassen’s matrix multiplication                                                               |
...
* **Matrix operations:** LU decomposition, LUP solver, Strassen’s multiplication
...
| Ternary Search    | O(log₃ n)              | O(1)             | More comparisons per iteration |
| Heap Sort         | O(n log n)             | O(1)             | In-place sorting               |
| Kruskal MST       | O(E log E)             | O(V)             | Efficient for sparse graphs    |
| Dijkstra          | O(V²) / O(E + V log V) | O(V)             | Single-source shortest path    |
